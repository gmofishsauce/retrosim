...
Why: the two panels dividing a fixed bottom region left neither with usable height, and each new bottom surface would have needed new divider arithmetic. A tab strip costs one row of chrome regardless of how many kinds exist, so the planned design-rule-check report (see `drc.md`) becomes a `TABS` row and a host element rather than a third stacked panel.
