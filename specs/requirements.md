...
- FR-071c: A built-in interactive input switch object. It occupies a 2×2 grid-unit footprint and exposes a single output connection point on the right edge (centered). It is drawn as a rotary dial — a circle bearing three position marks labeled 1, 0, and ? with a pointer indicating the current position — so the dial always displays its set value (unlike the indicator (FR-068), it is a signal source and shows its value whether or not a simulation is running). Its three positions correspond to the simulator's logic values: 1, 0, and ? (state U). Its palette tooltip is "input switch"; the same image is used for the palette icon and the placed object. The current position is per-instance state, persisted in the saved design and restored on load; a newly placed switch defaults to the ? (U) position. The switch declares no numeric properties (FR-020b). Its position may be set two ways: through the properties panel while editing (FR-020c) and, while a simulation runs, by clicking the dial (FR-087a). Its driving behavior is specified in §3.19 (FR-087a).
...
- FR-020c: When the single selected component is an input switch built-in (FR-071c), the properties panel (FR-020a) shall present a control for its position offering the three values 1, 0, and ? (U). Setting it changes the switch's persisted position state, is an undoable action (FR-024), and is disabled while a simulation runs (FR-087); during a run the position is instead changed by clicking the dial (FR-087a). The switch's position is per-instance interactive state rather than a numeric property (FR-020b), so it is not an `overrides` entry.
...
- FR-027: To draw a wire, the user shall click a source pin, then click a destination pin. The system shall create a wire between the two pins following the proposed route (FR-027c) current at the moment of the destination click. A press-and-drag alternative is provided by FR-027d. (Reworked 2026-06-12; supersedes the straight rat's-nest line — the committed wire now follows the Manhattan route proposal, with the straight line retained only as the router's fallback.)
...
- FR-027b: In select-tool mode, a component pin shall act as a wire hotspot, so the user can draw a wire without first activating the Wire tool. While in select-tool mode, hovering the cursor within a pin's hot region (FR-013d) shall change the cursor to the wire cursor (FR-025), and clicking there shall begin a wire from that pin exactly as the Wire tool does (FR-027/FR-027a); the wire is completed by the destination click and the application returns to select-tool mode (FR-028). Pressing and dragging from the pin instead completes the wire on release (drag-to-connect, FR-027d). Only pins are select-tool wire hotspots; clicks on wire segments, buses, bend points, and component bodies retain their existing select-tool meanings (FR-016, FR-031, FR-033b, FR-023a).
...
- FR-027d: As an alternative to the click-source, click-destination gesture of FR-027, the user may press the left mouse button on a source pin, drag, and release over the destination to commit the wire, following the proposed route (FR-027c) current at release. Drag-to-connect applies to pin sources only, in the Wire tool or via the select-tool pin hotspot (FR-027b); bus and wire-segment sources remain click-based (FR-039, FR-031). A press released without moving past the click threshold (FR-016a) is a click rather than a drag and behaves exactly as FR-027. On release after a genuine drag: releasing over a valid destination (pin, wire segment, or bus) creates the wire as in FR-027; releasing over empty canvas creates a wire from the source pin to a dangling free endpoint at the release point, snapped to the grid (FR-021, FR-029); releasing over the source pin itself cancels the gesture and creates nothing. After a drag-to-connect commit the application returns to select-tool mode (FR-028).
- FR-027e: While a wire is in progress (FR-027a) and the cursor lies within the hot region (FR-013d) of a valid destination pin other than the source, the application shall highlight that pin with a ring in the selection accent color, reinforced by non-color cues (the wire cursor of FR-025 and the route preview snapping to the pin) so the affordance does not rely on color alone. On a successful connection the application shall play a brief confirmation animation, a short expanding and fading ring under 200 ms, at the connection point. The confirmation animation shall be suppressed when the operating system requests reduced motion.
- FR-027f: A wire's dangling free endpoint (FR-029), whether dropped by drag-to-connect (FR-027d) or left after deleting a component, shall be draggable so the user can pick it up again. In select-tool mode, pressing within the hot region (FR-013d) of a free endpoint and dragging shall move that endpoint, snapping to the grid (FR-021). Releasing over a pin reconnects the wire's end to that pin (the drop highlight and confirmation pulse of FR-027e apply); releasing over empty canvas leaves the endpoint free at the release point. Only free endpoints are draggable this way: a pin endpoint follows its component (FR-018) and a junction is a shared branch point. The reconnect or move is a single undoable action (FR-024), and reconnecting removes the now-unused free vertex (FR-030 connectivity cleanup).
...
- FR-087a: The input switch built-in (FR-071c) is a strong driver: at every step it shall drive its output to the logic value of its current position — 1, 0, or U for the ? position (never Z), overriding any weak pull-up/pull-down driver on the net (FR-083). While a simulation is running (FR-076), clicking the switch's dial face shall advance it to the next position in the cyclic order ? → 1 → 0 → ?, changing its driven output and re-evaluating the simulation. The dial click is an interactive input (FR-087b): a deliberate exception to the read-only rule (FR-087) that updates the switch's persisted position state (FR-071c) and marks the design modified (FR-049a, FR-092) but is not placed on the undo stack (undo/redo remain disabled during simulation, FR-087). While not simulating the dial is not a click target: a click selects the switch as an ordinary component (FR-016), and its position is set through the properties panel (FR-020c).
- FR-087b: The simulator shall provide a general interactive-input mechanism for built-in objects that accept manual user input while a simulation runs. A built-in type may register an interaction handler — the input-side analogue of its behavior (FR-067a) — that mutates the instance's interactive state. While simulating, a click on such an object's body shall invoke its handler as a live, non-undoable design change (FR-087a) and shall cause the simulator to re-evaluate from the changed state (re-settling a combinational design per FR-085, or taking effect on the next step of a paced design per FR-086). The mechanism is general and not specific to any one object: a new interactive input is added solely by registering its handler (and render). The input switch (FR-071c/FR-087a) is the first interactive input object; clicks on non-interactive objects retain their normal simulating-mode meaning (selection, FR-087).
