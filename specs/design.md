...
- **Satisfies:** FR-061, FR-062, FR-062a, FR-062b, FR-062c, FR-062d, FR-062e, FR-062f, FR-063, FR-064, FR-066, FR-066a, FR-066i, FR-066j, FR-104.
...
- **GAL parts load leniently (FR-066i/FR-066j, 2026-09-13).** When the document
  declares `gal:`, every validation above that would return an error — a missing
  `type` or `partnumber`, a bad `gal:` name, a malformed pin `side`/`pos`/`dir`, a
  missing or duplicate pin name, a bad group (unknown or `NC` member, duplicate
  name, FR-063a geometry), a bad `clock:`, a bad `internal:` list, a `physical:`
  completeness violation, a `pos` outside the outline, and a bad `olmc` (below) —
  is instead **appended to `ComponentType.LoadErrors`** and parsing continues;
  `ParseComponent` returns an error for a GAL file only when `yaml.v3` cannot
  decode it. Continuing needs a defined value for each bad field: a pin with no
  name is dropped (nothing can key it); a pin with an unusable `side` or `pos` is
  kept with `Unplaced: true`, which drawing, hit-testing and wiring skip (the GAL
  dialog re-seats pins by `number` regardless); an unknown `dir` loads as `in`;
  duplicate pin names are both kept; a bad group is left out of `pinGroups`; an
  invalid `clock:` is carried as written. The per-pin `olmc` key (FR-066i)
  decodes onto `Pin.OLMC`: on a GAL part it must be `reg` or `comb` on a pin whose
  `dir` is not `in`, else a load error; on any other type it is ignored. Also for
  GAL parts only, top-level keys the parser maps onto no `ComponentType` field are
  captured, decoded, into `ComponentType.Extra` so the Edit GAL part dialog can
  write them back instead of dropping them (FR-066j); FR-066's "unknown keys are
  ignored" still holds for every other type. Non-GAL files keep every rejection
  above unchanged — nothing authors them in-app, and nobody expects a red
  74-series part.
...
- **Behavior:** decode JSON, delegate to `storage.go`/`components.go`, encode
  ...
  startup load, requires an authored marker — a `gal:` device (a GAL part,
  FR-066a/FR-066b) **or** a `mem` block (a memory device, FR-114f) — an immutable
  library-unique `id` (FR-066e), **and** a `project` directory (FR-121i) — a GAL
  submission's semantic problems are not 400s: it goes through the lenient GAL
  parse (§6.3), is written, and comes back with them in `loadErrors` (FR-066j),
  only undecodable YAML being a 400 for it — then
...
- **Data:** the `Design` object (§7.2), including its **`vertices`** collection
  ...
  - `addInstance(design, type, x, y, rotation) → instance` — assigns the
    ...
    edited only via `setLabelCmd` (§6.10). A placed **RAM** (a `mem` block with
    `kind:"ram"` and a `ramFile`, FR-114g) additionally has its save-file path
    **re-derived from the new refdes** here (FR-114h, helpers below), so a type's
    one path never becomes two instances writing the same file; the instance's
    derived path is what `placeComponent` captures for redo (§6.10).
  - **RAM save-file helpers (FR-114h)**, beside `addInstance` because placement is
    one of the two rules' application points and paste (§6.15) is the other, which
    imports them from here: `ramFileOf(inst)` — the instance's RAM save file or
    `null`, and deliberately *not* a ROM's `romFile`, which is shared read-only
    content (FR-114e); `takenRamFiles(design)` — every save file already spoken
    for in the design; and the pure `derivedRamFile(path, refdes, taken)` — split
    off directory and extension, strip a trailing designator suffix from the stem
    (`/-(?:U\d+[A-Z]*|A-\d+|N-\d+|X\d+)$/`, so copies of copies do not compound),
    append `-<refdes>`, then, while the candidate is in `taken`, insert `-2`,
    `-3`, … before the extension. Pure and unit-tested apart from either caller.
  ...
  - `refreshInstance(design, inst, libType, {noDrop}={}) → {ok, dropped[]} | {skip: reason}` (FR-088) —
    ...
    adopted from the library: a memory instance's `mem.romFile` (FR-114e) and,
    equally, a RAM's `mem.ramFile`/`mem.ramLoad` — the path FR-114h derived at
    placement or paste, so adopting the metatype's would put every refreshed
    instance back onto one shared file (FR-088); a
...
- **Simulation lock (FR-087):** while `store.state.simulating` the FSM accepts
  ...
  (FR-087a). Two generalizations came with the debug-input port (FR-094g, 2026-08-19):
  the hit must also **qualify** — a port is interactive only when this run's
  `sim.debugPorts` map lists it (§6.14), and one that does not falls through to the
  lock message like any other component — and the FSM passes the handler a **hit
  descriptor** `{bit}` alongside the instance, resolved for a `portN` as the index of
  the nearest pentagon center so each bit of a multi-bit port is its own click target.
  Neither is port-specific: they are the conditional-interactivity and sub-target
  clauses of FR-087b, and the switch handler simply ignores the second argument.
  A port additionally **yields to probe mode** — the qualification is skipped while
  `probeClaimsClick(state)` holds, so the click falls through to the probe branch below
  (FR-087c's port exception). The ordering is thus: switch → probe → port → lock message.
  (Reworked 2026-06-17; supersedes selection remaining available during a
...
- **Live inputs during a run (FR-087a/FR-087b, reworked 2026-08-05):** an interactive
  ...
  renderer's switch branch (§6.8) and the switch behavior's `state` (§6.13) — and, since
  2026-08-19, the port branches of both, reading the `portDrive` array a debug-input click
  leaves on the copy (FR-094g, §6.14). That field is **only** ever run-time state: unlike
  `switchState` it has no persisted counterpart on the instance, so an absent copy simply
  reads as undriven, and every run therefore begins with every port undriven. Lifetime is
...
- **Palette (rendered by `app.js` `renderPalette`; there is no separate
  ...
  **edited** in-app (FR-066f) replaces its library entry and re-renders its tile
  ...
  handler passes the type's **`id`** — never the type object captured when the
  tile was built — to the `onTileMenu` callback, which resolves it against the
  **live library** and builds items from what it finds: presently just "Edit part
  definition…", and only when that type is a project-local GAL part (`type.gal &&
  type.projectLocal`, §7.1) and no simulation is running (FR-087). Passing
  identity rather than a snapshot is load-bearing, not stylistic: an edit
  (FR-066f) replaces the library entry, so a captured object would keep handing
  the editor the definition as it stood at render time — the dialog would reopen
  showing the pre-edit part and, saved, would write that stale definition back
  over the file, silently reverting the edit. (Found in use immediately after the
  feature shipped, 2026-08-15; `replacePart` had updated the array entry and the
  tile's label but could not reach into the listener's closure.) With no items
...
- **Interactive-input registry (`INTERACTIONS`, FR-087b)** — a second registry
  ...
  *which* object: the FSM (§6.9) routes a simulating-mode click through
  ...
  the scheduler or the FSM. The **debug-input port** (FR-094g, 2026-08-19) is the second
  entry and the proof of that claim: `port` and `portN` share one handler advancing
  `inst.portDrive[bit]` undriven → `"0"` → `"1"` → undriven-or-`"0"` by the `releasable`
  flag the FSM passes (allocating the array on first touch; 0 first so reaching 1 is a
  `0→1` clock edge, FR-094g), and the only mechanism change it needed was widening the handler
  signature to `(inst, hit)` so the `portN` bit index can reach it (FR-087b, §6.9).
...
- **Properties panel (`properties.js`)** — Satisfies FR-011b, FR-020a, FR-020b, FR-105, FR-114i. A docked
  ...
  - *Memory section (FR-114i)*: for an instance whose `typeData.mem` exists, a
    **Memory** section after the doc block: read-only info rows for kind and
    geometry (`RAM`, `256 × 8`), then the file bindings — for a ROM a **content
    file** row, for a RAM a **save file** row plus a **load at start-up**
    checkbox. Each binding renders as a label + buttons row (`Choose…`, and for
    the RAM a `Clear`) followed by the path on **its own full-width line** (a
    dimmed placeholder when unset, the full path in its `title`). The path may
    not sit inside the flex row: a filesystem path offers no break opportunity,
    so as a flex item it either forces the panel wider than its column or, told
    to wrap anywhere, collapses to its min-content width of one character per
    line — which is what the first cut did. On its own line it wraps into the
    panel's full width. The buttons call
    `openFileDialog` (§6.11 `dialogs.js`) exactly as `memDeviceDialog` does —
    open-mode `exts:["bin","hex"]` for a ROM, save-mode with
    `saveExt:"bin"`/`saveExts:["bin","hex"]` for a RAM — seeded at
    `store.state.project?.dir` (FR-121h). The result dispatches
    `setMemFileCmd(refdes, patch)` (§6.10), a keys-in-`patch` command on the
    `setPortPropsCmd` pattern but writing `inst.typeData.mem` rather than the
    instance's own fields: it captures the prior values of just those keys once,
    so undo restores them. Clearing a RAM's save file sends
    `{ramFile: null, ramLoad: false}` in **one** command, so one undo restores
    both. The section is disabled under `store.isReadonly()` with the rest of the
    panel's edits (FR-087), and it is the only place a placed instance's derived
    save file (FR-114h) is visible after the creation-time tray line.
...
- **Dialect modes (FR-066a/FR-079a/FR-079b).** `compileBehavior` takes the
  ...
  **strict** for that device. The two modes share **one** evaluator — strictness
  ...
  - *Strict* (`gal:` present) runs a device table — pin/OLMC counts, which
    ...
    pass is one input to `galDefinitionErrors` (§6.14, FR-066j), which the
    New/Edit GAL part dialog runs live and which Run, Generate C… and the
    test-vector runner consult before starting — so the dialog reports exactly what
    Run would refuse, without refusing to save it (2026-09-13; the dialog formerly
    used this gate to block Create/Save).
...
- **Compile at Run:** `buildNets(design)` (§6.6) gives the net partition; build a
  ...
  **Subunit packages:** siblings sharing a U-number (§6.6) form one evaluation
  ...
  sibling that owns its pin. Before `buildSimulation` is called at all, `createSim`'s
  `run()` runs `designDefinitionErrors` on the flattened design (§6.14, FR-066m)
  and, when any GAL instance has definition errors, hands the message to its
  `onRefusal` hook — which `app.js` binds to the modal — instead of starting, so
  for a GAL part the preflight failures below are a backstop. Both engines treat
  a behavior block of nothing but `;` comments as no behavior (`hasEquations`). Preflight failures post to
  ...
  reported once and their outputs drive U for the run (FR-080); so, for a GAL
  part, does every output-capable pin (`direction !== "in"`) its compiled
  behavior writes no equation for — those pins join the entity's `uPins`, and
  the once-per-type report names them (FR-080, 2026-09-13; previously undriven).
...
- **Built-in behaviors (FR-067a):** the `BEHAVIORS` registry entries (§6.11)
  ...
  **previous** step's net values, so an input-reading built-in's outputs follow
  ...
  parity harness pass nothing and read the instance. `drive` comes from the run-time copy **alone** —
  `liveInputs?.(refdes)?.portDrive`, with no `?? e.inst` fallback, unlike `state` — since
  a port's drive is never part of the design (FR-094g). A runner that passes no accessor
  therefore sees every port undriven whatever its instances carry, which is exactly what
  the vector runner and the parity harness require; the renderer reads it the same way
  (§6.8), so neither can be fooled by a stray field on an instance. The simulator entity still
...
- **Display view:** the engine publishes `state.sim = { valueOfPin(refdes,
  ...
  and the inputs the user set. `debugPorts` — this run's eligible-port map, resolved
  once from the **root** design at Run (FR-094g, §6.14) — rides along for the same
  reason the `inputs` map does: it is true only of this run, and both the renderer and
  the click FSM must agree on it. That is why **`stop()` calls `store.setSim(null)`**
  (FR-085): one call blanks the indicators, reverts the clicked switches, returns every
  debug-input port to a plain white flag, drops the
...
**The dialog's tabbed structure (FR-066h, 2026-09-04).** `newGalPartDialog` renders four tab bodies — **Part** (part number, description, the two pin-field lists with the pin-1 clock checkbox, the pin-groups row), **Logic** (the term grid, the kept-equations list and the GALasm preview), **Notes** (one textarea over `part.notes`), **Errors** (a read-only scrolling `<textarea>` of the current `galDefinitionErrors` lines, then the load notes under their own heading — FR-066k, added 2026-09-13) — with the title, the status line (which carries the red one-line definition-error summary, FR-066k), the inline submit-error line, and the Cancel/Create-or-Save buttons **outside** the strip and visible from every tab. Mechanically this is a `hidden` toggle over three `<div>`s plus a `selectTab(key)`; no field moved between owners, `gather()`, `validate()`, `tablePins()`, `renderEq()`, `behaviorText()`, and `galPartYaml` are untouched, and the grid still rebuilds on a label or direction edit made on a *different* tab, because those listeners were never coupled to visibility. Two rules are worth stating because they are the ones a tabbed dialog gets wrong: the check judges the **whole part**, never the visible tab — an equation error found while the user is on Notes still puts the red summary in the status line (FR-066k), though since 2026-09-13 it no longer disables Save — and a **refused submit selects the tab carrying the problem** before reporting it (`onOk` picks Part for a missing part number on Create, the one local refusal left, FR-066j), so no message ever names something off screen. A server-side submit error names no tab and moves no selection. The reason for the change is capacity, not taste: the dialog had eight stacked regions in a `92vh` box, three of them scrolling and competing for height, and FR-125a's notes area is what made a single column untenable.

**The New GAL part flow (FR-066b/066c/007a).** `builtins.js` exposes a non-placeable upper-palette action **New GAL part** (a tile that opens a dialog rather than arming placement). The **New GAL part dialog** (`dialogs.js`) renders the device's fixed skeleton — for the GAL22V10, the 24-pin map (pin 1 clock/in, 2–11 + 13 in, 14–23 OLMC I/O, 12 GND, 24 VCC) — and collects only the per-part data: `partnumber`, optional `description`, a label per I/O pin, a per-OLMC direction (in / comb-out / reg-out — saved as `dir` plus a per-pin `olmc`, FR-066i), a pin-1 **clock** checkbox (saved as `clock:`, FR-066i), optional named pin groups (FR-066d, below), and the part's logic, clicked into the **equation term table** (FR-066g, below) from which the `behavior` block is generated — there is no equation text box. As the user types or clicks, the dialog assembles a candidate `typeData` (`type:"22V10"`, `gal:"GAL22V10"`, an immutable `id` generated from the `partnumber` (FR-066e), the chosen `pins`, the `behavior`) and runs `galDefinitionErrors` (below) on it **live** — which wraps `galasm.js` `compileBehavior`+`validateStrict` (§6.13), so the diagnostics are the ones Run would give (FR-079b) and there is still no second behavior validator — reporting them on the Errors tab without blocking OK (FR-066j/FR-066k). OK serializes the `typeData` to YAML client-side and `POST`s it to `/api/v1/components` **with the current project dir** (`store.state.project.dir`), so the server writes the `.yaml` under `<project>/components/` (FR-007a/FR-121i); on success it dispatches the live palette add (above) and returns to SELECT (one-shot, FR-010). A duplicate-`id`/existing-file 409 — collision against the project `components/` **or** the shared library (FR-121i) — is shown in the dialog; Cancel discards. Placement of the resulting tile is then ordinary FR-008/FR-009.

**The Edit GAL part flow (FR-066f/FR-006b/FR-007a/FR-088).** The same `newGalPartDialog` serves as the editor, opened with the part to edit instead of a blank skeleton — one dialog, one set of fields, one validation gate, differing only in its title, its **Save** (vs. Create) button, and whether the submit posts `mode:"update"` or `mode:"create"` (§6.4). Entry is the palette tile's context menu (FR-006b, §6.11) **or** the canvas context menu on a placed instance of the part (FR-033b, §6.11), whose "Edit part definition…" item exists only for a `type.gal && type.projectLocal` type (§7.1) with no simulation running (FR-087) — resolved from the live library by `id` at click time, so a second edit in one session starts from the first edit's result rather than from a stale snapshot (§6.11). The instance route resolves through the same `findType(typeIdentity(inst.typeData))` the placement code uses: the instance supplies the **identity** and the library supplies the **data**, so an instance still carrying a pre-edit `typeData` copy (FR-057) opens the current definition, and an instance whose type is absent from the library (a design opened outside its project) offers no item. Both routes call one `onEditGalPart(type)` in `app.js`; `interaction.js` owns no part of the edit beyond deciding that the item applies. (Instance route added 2026-09-04: the tile was the only entry, and neighbouring tiles for similarly-named project parts are easy to confuse — the chip on the sheet is the unambiguous referent.) **Load-back** is a pure `galPartFromType(type) → {fields, loadNotes}` (`dialogs.js`, testable without DOM) that **never refuses** (FR-066j, 2026-09-13: it used to return `{refuse: reason}`, which `app.js` posted as an "Edit the YAML directly" tray message — both are gone, and the device test moved into the menu's eligibility, `type.gal === "GAL22V10"`, FR-006b). It maps a loaded `ComponentType` onto the dialog's model as best it can and lists, as load notes, whatever it supplied or kept: skeleton pins are matched by `number` — a file pin at the wrong side/`pos` takes its skeleton slot (noted), a skeleton slot with no file pin takes the skeleton's default label (noted), and a file pin with no number or a number the skeleton lacks goes to `keptPins`, re-emitted verbatim after the skeleton pins (noted); each OLMC's type comes from `Pin.olmc` when present and otherwise from whether the behavior registers it (the pre-FR-066i inference); the clock checkbox is `type.clock === <pin-1 label>` (a `clock:` naming anything else is already a load error, and loads unchecked); `pinGroups` resolve to skeleton DIP numbers, a member naming no pin being dropped from its group (noted); every `ComponentType` field the dialog does not model (`delays`, `internal`, `datasheet`, `physical`, pin `desc`, a non-skeleton `width`/`height`) plus `type.extra` is carried in `kept` and re-emitted by `galPartYaml` as YAML keys with JSON flow-form values (JSON being YAML), so none of it is dropped (noted); and the `behavior` text is parsed into the term table by `galeq.js` (below), whose unrecognized equations come back as kept equations rather than as a refusal. Rewriting the file whole on Save stays safe for the reason the refusal used to guarantee — nothing the dialog read is lost — YAML comments excepted, which no in-app write has ever kept (FR-066f). **Save** serializes through the very same `galPartYaml` the create path uses — which since FR-066i writes `olmc:` on every output-configured OLMC and `clock:` from the checkbox, never from "some OLMC is registered" — and posts it with `mode:"update"`; `api.js` gains `updateComponent(yaml, projectDir)` beside `createComponent`, and a 404/403/400 surfaces inline exactly as a create's 409 does, leaving the dialog open. On success `app.js` **replaces** the type in the client library (by `id`) and re-renders the tile (§6.11), then dispatches `RefreshTypes` **filtered to that `id`** (§6.10) — instances of the edited part adopt the new pins, directions, groups, and behavior; connections to a pin the edit renamed or removed are dropped and reported (FR-088); the whole refresh is one undo step. The YAML write is **not** in that step: undo restores the schematic, never the file, and the tray report says so implicitly by naming what it changed. Instances in other designs on disk are untouched until each is opened and refreshed on its own (FR-088) — the definition, not the `typeData` copy embedded in a save (FR-057), is authoritative.

**The equation term table (FR-066g).** The dialog's logic region is a grid of cycling buttons, not a textarea, and the translation between it and GALasm text lives in a new pure module `web/js/engine/galeq.js` (DOM-free, unit-tested on its own).

*Model.* `{ outputs: Map(olmcNumber → { mode:"eq"|"const0"|"const1", reg, note, rows:[ Map(pinNumber → "1"|"0") ] }), kept: [ text ] }`. `reg` is the output's own comb/reg control (FR-066g), deliberately independent of the OLMC's declared type so the two can disagree and the disagreement survive Save (FR-066i); `kept` holds the equations the table cannot represent, verbatim with their comments, in source order (added 2026-09-13). Everything is keyed by **skeleton DIP number**, never by label — the same rule that keeps a pin group intact across a rename (FR-066d) — so relabeling a pin mid-dialog preserves every term that names it; labels are resolved only when the text is emitted. A row is a sparse map: a signal absent from it is the `X` cell, so an empty row is an empty map, and a term is never larger than the literals actually in it.

*Emit.* `tableToBehavior(model, { labelOf })` walks the OLMC pins in pin order 14→23 and writes each output that has something to say: `const0`/`const1` emit `LHS = GND` / `LHS = VCC`; `eq` drops empty rows and, if any survive, emits one AND term per line joined by leading `+`, with `*` between literals and `!` for a `"0"` cell. The LHS is the pin label verbatim (so a `/ENF` pin heads `/ENF = …`, FR-066g) plus `.R` when the output's own `reg` is set (not when the OLMC's declared type is registered — FR-066i); RHS literals use the label with any leading slash stripped, §7.6's physical-level convention. The output's note, if any, is appended to its first line as `; …`. An output with no surviving rows emits nothing at all. After the last table output each `kept` text is appended verbatim, in order — which also keeps an `.E` after the output it enables.

*Parse.* `behaviorToTable(text, { pins })` is the inverse, and since 2026-09-13 it **refuses nothing**: the text is split into equation chunks — a chunk opens at a line that starts a new left-hand side and runs through its `+` continuation lines, carrying its `;` comments — and each chunk the recognizer accepts becomes table rows (setting its output's `reg` from its `.R`) while each it declines becomes a `kept` entry (FR-066g). It cannot reuse `galasm.js`'s `tokenize`, which discards `;` comments and line structure — both of which the table keeps (FR-066g's per-output notes) — so `galeq.js` carries a small line-aware tokenizer of its own and a deliberately **narrow** recognizer: only `[/|!]NAME[.R] = <sum of products>` and `NAME = VCC|GND` are accepted, which is what makes every unsupported form (`.E`, `.T`, `.L`/`.G`, `.CLK`/`.ARST`/`.APRST`, `:+:`, `AR`, `SP`) a kept equation rather than a silent drop. It further declines an LHS whose negation disagrees with its pin's label (the polarity lives in the label, FR-066g, so loading `!OUTCMB = …` on a pin labeled `OUTCMB` and saving would invert the output), an output whose term reads its own signal back, and a term naming one signal in **both** polarities — except where that contradiction is the output's only term, which is exactly the idiom for a hard 0 (`S2 = F0 * !F0`) and loads as `const0`. It also declines an LHS naming no OLMC pin (a fixed input, `NC`, an unknown name) and a second accepted chunk for an output that already has rows; the duplicate definition that leaves in the emitted text is the compiler's to report, as a definition error. A chunk for an OLMC currently configured as an input is accepted, and its rows are shown and reported (FR-066g). A literal repeated in the same polarity within a term collapses onto its one cell. Comments are attached to the output whose equation encloses or follows them, the trailing ones to the last output. The whole emitted text, kept equations included, is then checked by `galDefinitionErrors`, so `galeq.js` never has to duplicate `compileBehavior`'s judgment — it only has to decline what it cannot represent.

*DOM.* `dialogs.js` builds the grid from the current labels and directions: a sticky header row of column labels (pin order, `NC` pins excluded), then per output a contiguous run of rows carrying the output's label, its equation/always-0/always-1 mode select, its **comb/reg** select (FR-066g), and its note field on the first row, one cycling `X/1/0` button per column, the positional **OR/done** drop-down, and the trailing **✕**. The self-column cell is inert, and while the pin-1 clock checkbox is set the pin-1 column takes no *new* literal, though an existing one stays visible and is reported (FR-066g). The grid is rebuilt (never re-keyed) whenever a label or direction changes, which is why the model is keyed by DIP number: rows for an OLMC switched to `in` stay **visible** while they hold content — they will be saved, so they are shown, and `galDefinitionErrors` reports them — and an output with no content simply leaves the grid; an output's comb/reg select follows the declared type only while the output has no content (FR-066g). Below the grid sits a **read-only** `<pre>` preview of `tableToBehavior`'s output, refreshed on every edit, and beside it a read-only list of the `kept` equations, each with a ✕. The candidate `typeData` takes its `behavior` from `tableToBehavior`, and `galDefinitionErrors` (below) reports on it live without gating Create/Save (FR-066j).
...
**GAL definition errors (FR-066j–FR-066m, 2026-09-13).** One pure module, `web/js/engine/galerrors.js`, owns the question "does this GAL definition have errors?", so the dialog, the canvas and the three refusals cannot disagree. `galDefinitionErrors(typeData) → string[]` returns `[]` for a type without `gal:` and otherwise, in order: the server's `typeData.loadErrors` (§6.3); the `compileBehavior` + `validateStrict` messages (§6.13) for a non-empty `behavior`; `.R` without `clock:`; the `clock:` pin's signal used as a literal; and, for each pin carrying `olmc`, a mismatch with the compiled behavior (`olmc: reg` on a pin written without `.R`, `olmc: comb` on one written with `.R`) and `olmc: reg` on a part with no `clock:`; plus any compiled output whose pin is `dir: in`. A pin without `olmc` is judged only by the checks that need no declaration, and a declared output with no equation yields nothing (FR-066j). Results are memoized in a `WeakMap` keyed by the `typeData` object: instances share one type-data object until Refresh Types replaces it (§6.10), so a sheet of many instances costs one check per type. `designDefinitionErrors(flat, root)` reads the design `flatten` produced — which prefixes each child part's refdes with its instance path (`X1/U6`) — and returns `{ parts, subs, sheets }`: a root-sheet GAL instance with errors contributes its designator, a part under a prefix naming a root sub-design instance contributes that instance's designator (at any depth, without saying which part), and a part on a merged peer sheet contributes the sheet's tag. `definitionErrorsMessage(lead, result)` words the refusal, or returns null. Consumers: the dialog's Errors tab and red status line (FR-066k); `canvas.js` `drawComponent`, which uses `CONFLICT_COLOR` for the body stroke and both labels when the instance's type has errors, in place of `#333` and of the selected `#4a90d9`, keeping the selected 2px width (FR-066l); `interaction.js`, which sets `canvas.title = "component definition contains errors"` while `state.hover` names such an instance and clears it otherwise; and `sim.js` `createSim().run()` (through `onRefusal`), `app.js`'s Generate C…, and the test-vector panel's run/capture, each of which checks the flattened design before starting and on a non-empty result opens `definitionErrorsDialog(message)` (`dialogs.js`, a modal with one OK) instead. One cause is one sentence ("The design cannot be run because the definition of U6 contains errors"); several are listed under "The design cannot be run because:" (FR-066m).
...
**Ports as debug inputs (FR-094g) — `debugPorts` in `subdesign.js` + store/canvas/interaction/sim.** A port on the sheet being run may be clicked to drive its own net, so a design meant for embedding can be exercised without gutting its interface. Nothing new is invented: the state rides the FR-087b interactive-input mechanism (§6.10 `setLiveInput`), the drive rides the existing built-in behavior path (§6.13), and the colors are the indicator's (§6.8).
  - **Eligibility — `debugPorts(design) → Map<refdes, {bits, releasable}>`** (beside `portDirection`, one `buildNets` pass for the whole sheet rather than `effectivePortDir`'s per-port pass). A port qualifies when it has no `target` (FR-101) and its effective direction — `applyOverride(classifyPortDir|classifyPortNDir(...), inst)`, the same derivation `effectivePortDir` performs — is `in` or `bidir`, never `out`. `bits` is `1` for a `port`, `typeData.pins.length` for a `portN`. **`releasable` is `dir === "bidir"`** and selects the handler's cycle (FR-094g): three-state for a bus port that must be able to let go, two-state 0↔1 for an input port, whose net has no other driver to let go *to*. Direction thus decides both *whether* a port is clickable and *how* it cycles, from one derivation — and FR-094d's override, already the user's control over an ambiguous bidir, becomes their control over the cycle too. It is called on the **root** design (`store.design`), never on the FlatDesign, which is exactly what makes a child's ports unclickable: `flatten` prefixes them and the canvas never draws them.
  - **Publication.** `createSim.run()` computes it after a successful `buildSimulation` and hands it to `store.setSim({ ..., debugPorts })` (§6.10), so it has the sim view's lifetime: created per run, dropped whole at Stop with `inputs` and the probe target. Both readers — the renderer, every frame, and the FSM, per click — therefore consult one resolved-once map rather than re-deriving from the wiring, which the read-only rule (FR-087) makes safe.
  - **Run-time state.** The drive lives on the run-time copy in `state.sim.inputs[refdes]` as `portDrive`: an array of length `bits` whose entries are `"1"`, `"0"`, or absent/`null` for undriven. An array in both cases (length 1 for a 1-wide port) keeps one shape for the renderer, the behavior, and the handler. Nothing seeds it — an absent copy or an absent `portDrive` reads as all-undriven, which is why every run starts undriven with no reset step, and why `setSim(null)` at Stop is the whole of FR-094g's lifetime rule (§6.10).
  - **Interaction (§6.9).** The simulating-mode click already routes a hit component with an `INTERACTIONS` entry through `store.setLiveInput`. Ports now have one, so the FSM adds a guard and an argument: a port hit is interactive only if `sim.debugPorts.has(refdes)` (else it falls through to the lock message, FR-087), and the FSM resolves **which bit** was clicked before dispatching — for a `portN`, the index of the nearest pentagon center, `rotateOffset(1.85, i + 1, inst.rotation)` offset from the instance origin, matching `drawPortN`'s row layout; `0` for a 1-wide port. Handlers therefore take `(inst, hit)` rather than `(inst)`, with `hit = { bit }`; the switch handler ignores it (FR-087b's generalization). A port also yields to probe mode, which a switch does not (FR-087c): the port branch is skipped while `probeClaimsClick` holds, so the click reaches the probe below.
  - **Handler (`INTERACTIONS`, §6.11).** The `port` and `portN` entries share one function advancing `portDrive[bit]`, allocating the array on first touch: undriven → `"0"` → `"1"` → (undriven if `hit.releasable`, else back to `"0"`). As with the switch, the handler just mutates the object it is handed; it does not know it is a copy, and it does not know a port's direction either — the FSM passes `releasable` through from the map, so the direction is derived once at Run rather than per click. **0 precedes 1 by requirement, not taste** (FR-094g): a rising edge is strictly `0→1` (§6.13, FR-079), so this order is the only one that makes reaching 1 an edge, and thus the only one that can hand-clock a design through a port. Reversing it silently makes a clock port useless — the failure mode that produced the rule.
  - **Drive (§6.13).** The `port`/`portN` behaviors stop being unconditional `[]`. `step()` already resolves the effective instance (`liveInputs?.(e.refdes) ?? e.inst`) for every `kind:"builtin"` entity; its context gains `drive: src.portDrive`, and the behaviors return one contribution per driven bit — `{pin:"P", value}` for a port, `{pin:"P"+i, value}` for a portN — omitting undriven bits entirely, so `add()` never sees them and the net's driver set is unchanged from today's. The contribution is **strong**: a debug drive beats a pull-up (FR-083) and conflicts with an enabled output exactly as two switches on one net do. A 1-wide port's `P` is a `connector` vertex joined by label (FR-094a), which `buildNets` already puts in `netOfPin` (FR-094e), so the contribution lands on the labelled net with no netlist change.
  - **Render (§6.8).** `drawPort`/`drawPortN` take the sim view and pick the body fill per bit from the same table the indicator uses: white/black label for `"1"`, black/white label for `"0"`, medium gray for undriven — but **only** when `sim.debugPorts` lists the refdes; an ineligible port and a port outside a run keep today's `#fff`. `drawPortN` already fills each pentagon separately, so the per-bit case is one lookup inside its loop.
  - **What does not change.** `cgen` (§6.17) reads the design as authored, where a port drives nothing, so the generated C and FR-107 parity are untouched — the same reasoning that keeps the probe out of it (FR-087c). The vector runner (§6.16) drives ports through `stimulus` (FR-115f) and publishes no `debugPorts`, so a held run (FR-115l) offers no port clicks. `designInterface`, `flatten`, `buildNets`, the DRC (§6.21), and the save format (§7.2) all see exactly what they see now.
...
- **Child-sheet path resolution (FR-098/FR-121g):** `loadSheet` — the one place
  `flatten` reads a child design — converts that sheet's stored **relative**
  paths against the **child's own** directory: each nested sub-design's
  `childPath`, and its mem data paths (`typeData.mem.romFile`/`ramFile`) via the
  same `absolutizeDataPaths` (§6.19). Both conversions belong here because
  nothing else performs them for a child: `loadChild` is `api.loadDesign`, which
  returns the file verbatim, and `fileops`' `loadIntoStore` only ever sees the
  **root** design. The data-path half was missing until 2026-09-21, and the
  consequence was total rather than subtle — a child's ROM reached the run-time
  loader still relative, the server refused it ("path must be a non-empty
  absolute path"), `loadRomContents` reported and skipped it (FR-114e's
  non-fatal rule), and the device read U, so an embedded CPU fetched U
  instructions forever. Resolving against the child's directory, not the root's,
  is what FR-121g means by relative-to-its-own-design: a child one directory down
  names its ROM relative to itself.
- **Interface-pin aliases (FR-087c).** `flatten` carries a `pinAliases` map on the
  FlatDesign: `<X-refdes>.<interface pin>` → `<flat refdes>.<pin>`, filled from the
  same `stitchMap` that rewrites the attachments, for every embedded instance at
  every depth. It exists because flatten **removes** the instance — `X1` is not a
  component of the flat design — while the probe (FR-087c, §6.13) hit-tests the
  *schematic*, where `X1` is exactly what the user clicked. Without the map
  `valueOfPin("X1","FTAB")` finds no such pin and returns its no-net default, `Z`;
  every interface pin of every sub-design therefore read Z, including pins visibly
  wired to a pull-up, while the simulation itself was correct — the parent's wire
  had been stitched to the child port and carried the right value all along.
  `buildSimulation` applies the map once at build, after `netOfPin` is populated
  and only for keys it does not already hold, so an alias can add a reading but
  never shadow a real pin's own net. The map is simulation-only, like the
  FlatDesign: never saved (§7.2), never exported (§6.18). (Added 2026-08-15; the
  code had never implemented FR-087c's "an embedded sub-design instance shows the
  values of its **interface pins**", and nothing caught it because the electrical
  result was right and only the readout was wrong.)
- **Stale-attachment rewrite (FR-099b):** a child's ports may have been renamed
  since the parent was saved, so once `resolveSubDesigns` has replaced an
  instance's synthetic `typeData` it sweeps the parent for attachments naming
  interface pins that no longer exist, in the **two** places a `(refdes, pin)`
  reference can live (the same two `repairStructure` checks, §7.4): a
  `pin`/`connector` **vertex** becomes a `free` vertex, keeping its last-known
  point (`ref`/`pin` deleted); a bus **`groupConnections` entry** whose `bitMap`
  names any missing pin is dropped **whole** — never per-bit, which would
  silently change the bus's topology — leaving the bus and its endpoint vertex
  dangling. Each is reported to the tray (FR-074). This sweep must live *here*
  rather than in `repairStructure`, which runs before the interface exists and
  therefore exempts sub-designs (`hasPin` returns true for them); the
  group-connection half was missing until 2026-08-10, and because
...
- **Satisfies:** FR-111, FR-112, FR-112a, FR-113; extends FR-004a (Edit menu), FR-016a (selection), FR-024 (undo).
...
**Pasting the fragment (FR-112).** `pasteFragment(design, fragment, dx, dy) → { components, wires, buses }` instantiates the fragment into `design`:
  ...
  - **RAM save files (FR-112a/FR-114h).** A pasted component whose `typeData.mem` is a RAM with a `ramFile` gets a **fresh** path from `derivedRamFile(path, newRefdes, taken)` — the same helper placement uses, imported from `model/design.js` (§6.6) rather than duplicated here, so the two application points of FR-114h cannot drift apart. `taken` starts as `takenRamFiles(design)` and grows as the paste assigns them, so one paste of several RAMs cannot collide with itself. `ramLoad` is untouched, and `romFile` is never rewritten (FR-114e content is shared by design). The rewrites ride back out on the return value as `ramFiles: [{ refdes, from, to }]`.
  Per-instance `typeData`, `overrides`, and `switchState` otherwise clone verbatim (FR-057/FR-058/FR-071c). The pasted objects are returned so the caller can select them.

  The **command** `pasteFragmentCmd(fragment, dx, dy)` (§6.10) is snapshot-based — it touches `components/wires/buses/vertices` plus the id counters, exactly the `snapshotConnectivity` set — so it reuses the existing `snapshotCommand` machinery. Capture on first apply, restore on revert — but redo does **not** re-run `pasteFragment`: the FR-011c refdes counters are monotonic and are not restored by the snapshot, so a re-run would allocate *different* designators and break any later redone command that names the first-apply refdeses. The first apply therefore captures everything it appended (components, wires, buses, vertices, and the post-paste id counters) and redo replays that verbatim — which also makes the FR-112a save-file derivation stable across undo/redo, since the derived paths are part of what is replayed. It exposes the created components' refdeses (captured on apply) so the FSM can set the post-paste selection, and the FR-114h rewrites (`ramRenames`) so it can report them. `placeComponent` (§6.10) exposes `ramRenames` the same way, from the derivation `addInstance` performed, and `interaction.js` reports both through one helper — the tray line is the user's only notice that the file is not the one named on the type.

**Paste-at-cursor FSM (FR-113).** `interaction.js` (§6.9) gains a `paste` placement mode paralleling `place`: `startPaste()` checks the clipboard is non-empty and not simulating (FR-087), sets `tool === "paste"`, and arms a floating ghost. `mousemove` computes `(dx,dy)` so the fragment's anchor — the grid-snapped top-left of its components' bounding box — tracks the cursor grid point, and calls a new renderer hook `renderer.setGhost(fragment, dx, dy)`. A left-click commits `store.dispatch(pasteFragmentCmd(fragment, dx, dy))`, selects the created components, clears the ghost, and returns to select-tool (one-shot, FR-010); any `cmd.ramRenames` the paste produced are then reported to the message tray (FR-074/FR-112a) — one line naming the instance and its new file, or a count when a single paste freshened several. Escape or any `setTool` clears the ghost and cancels (no mutation). The **ghost** is a new translucent-draw path in `canvas.js` (§6.8): with `ctx.globalAlpha` lowered it draws the fragment's components through the existing `drawComponent` (offset by `dx,dy`) and its conductors as plain polylines — read-only, never hit-tested, drawn above the design and below the marquee.
...
- **Satisfies:** FR-106, FR-107, FR-108, FR-109, FR-110, FR-110a, FR-116, FR-116a, FR-117, FR-117d, FR-118; extends FR-004a (new Tools-menu item; the menu was named Simulate until FR-124h).
...
**Runtime (`web/cgen/runtime.h` / `web/cgen/runtime.c`).** Checked-in, human-readable C (C99, no dependencies beyond libc), served as static assets by the existing Go static handler and fetched by the client at generate time. `runtime.h` is the documented API (comments written as future user documentation); `runtime.c` carries FR-number references at each semantic re-expression, mirroring `sim.js`'s comment style. Core pieces:
  - **Four-state type:** `typedef enum { RT_0, RT_1, RT_U, RT_Z } rt_val;` — numerically identical to `V0`/`V1`/`VU`/`VZ` (§6.13). One byte per net in a single `rt_val curr[]` array (it was double-buffered `curr[]`/`next[]` until M13; the unit delay of FR-078 is kept by ordering, since every driver reads `curr` before any net is resolved into it); bit-packing is a deliberately deferred optimization.
  - **Combination ops:** `rt_and`, `rt_or`, `rt_not`, `rt_xor` implementing FR-077 selective pessimism (`0 AND x = 0`, `1 OR x = 1`, other U → U; Z reads as U), used by the generated evaluate functions. They (with `rt_buf` and the Z→U read normalization `rt_norm`) are defined **`static inline` in `runtime.h`**, not in `runtime.c`: they are the innermost operations of every generated evaluate function and dominate the profile, and a call across the two translation units cannot be inlined without link-time optimization — measured on `examples/cpu` at about 100 clocks/s at `-O2` out-of-line versus about 1,000 with `-O2 -flto`. Defining them in the header lets any optimizing compile inline them. (Moved from `runtime.c` 2026-09-23.)
  - **Driver slots + resolution:** every driver owns one fixed value slot (M12); the generated per-step code writes it with `rt_drive(slot, val)` (Z = not driving), the runtime's built-in and memory drivers likewise, and each net resolves from its own slots, listed in contribution order by the generated `gen_net_slot_start` CSR table. (Reworked 2026-09-24: supersedes per-step `rt_contrib(net, val, weak, label_index)` contribution lists.) Resolution re-expresses `resolveNet` from `sim.js` — enabled strong drivers win, weak pulls decide only when every strong driver is Z, 0-vs-1 disagreement → U with a conflict report to stderr naming both drivers on onset (FR-081–FR-083, FR-108/FR-118).
  - **Step/settle:** `rt_step()` (latch phase → evaluate dirty units → built-in and memory drivers → resolve dirty nets in place, FR-078/FR-110/FR-110a; M13 below; reworked 2026-09-24, superseding latch → contributions → resolve every net → swap) and `rt_settle()` (step to quiescence under `RT_SETTLE_BOUND` 10000, FR-085's bound).
...
**Milestones.** (Sequencing per the 2026-07-02 discussion recorded in `gen-open.md`.)
  ...
  10. **M10 — free-run quiescence skip** (scoped 2026-09-23: FR-117d). On `examples/cpu` only 7.7% of free-run unit steps change any net: a 100 ns clock period settles within a handful of units after each edge, and every later unit re-evaluates the whole design to the same answer. **Fixed-point detection:** `rt_step` computes the net-change flag exactly as before (its return value, which `rt_settle` depends on, is unchanged) and separately records whether the step was a full fixed point — no net changed **and** the latch phase changed no state. The latch phase reports this itself: `gen_latch` now **returns `int`**, nonzero when any generated state (`reg_`, `regprev_`, `prevClk_`, `latch_`) took a different value, via a generated `rt_upd` store that compares before writing; `mem_write_all` and `uart_step` report a change to `prev_we`/`uart_prev_clk` (a store write or UART byte happens only on such an edge, so it implies one). This is a `gen_`/runtime **interface change**: programs generated earlier must be regenerated (the M7/M8 precedent). **The skip:** `rt_run_free` loops `while (sim_time < end)`, and after a fixed-point step evaluated at time t it sets `sim_time` to the earliest t' > t at which a time-driven built-in's output differs from its output at t, capped at `end`. A clock's output is `(t mod p) < p/2` (p clamped to ≥ 2, the `drive_builtins` rule), so its next change is the next multiple of p/2 boundary: `t - r + p/2` when r = t mod p < p/2, else `t - r + p`. A reset's is `cycles × clockPeriod` while t is below it, else never. Switches, pulls and port stimulus are constant in free run. The steps skipped are exact no-ops: they would recompute the same contributions and nets, rewrite the same conflict flags without a new onset report, sample no VCD change, and latch nothing. **Why net quiescence alone is not enough:** `gen_drive` reads a register's feedback through `regprev_`, refreshed at the top of each latch phase. A register that latches behind a disabled (`.E`) output changes no net in that step, but the next step's `regprev_` differs and can change a combinational output. Checking the latch state closes that gap exactly, and costs only the latch phase, which is ~1% of the profile. **Verification:** the FR-107 parity legs, which compare free runs against the slow simulator; plus the `examples/cpu` fib image's final dump and a VCD trace of the whole run compared byte-for-byte against the pre-M10 runtime.
  11. **M11 — free run by default; `-v` selects vectors** (scoped 2026-09-24: FR-117, FR-117a). `main()` picks the mode from its flags: `-v`/`-vectors`/`--vectors` → `rt_run_vectors()` on stdin; otherwise free run, bounded by `--cycles N` or, without it, unbounded. `-v` with `--cycles` is a usage error. `--columns`, `--vcd` and `--rom` are unchanged. **Unbounded run:** `rt_run_free(0)` means no end. A SIGINT handler (installed only for free runs, so a vector run still dies on Ctrl-C) sets a `volatile sig_atomic_t` stop flag. The step loop checks it before each step, so the step in progress completes and the run then ends through the ordinary bounded-run exit path: UART flush, `LABEL=v` dump, `mem_save_all`, VCD close, exit 0. A SIGINT during a bounded run ends it early by the same path. When the quiescence skip (FR-117d, M10) finds no future built-in change (`next_builtin_change` returns `LONG_MAX`) in an unbounded run, the design can never change again, so the loop sleeps (`sleep(1)` until the flag is set) instead of spinning. `sleep` is the one POSIX call in the runtime, beside C's `signal`. **Tooling:** `parity.js` passes `-v` on its vector legs, and `tv2txt`'s usage line becomes `node tv2txt.js ./sim design.tv | ./sim -v`. A program generated earlier keeps its own `runtime.c` and so its old behavior until regenerated.
  12. **M12 — fixed driver slots** (scoped 2026-09-24: FR-116a amended). Profiling `examples/cpu` at `-O2` after M10 put about 14% of run time in the out-of-line `rt_contrib` and about 26% in `rt_step`, mostly resetting `head[]` and walking per-net linked lists of contributions rebuilt every step. **Slots:** the generator records every driver, in the global **contribution order** (the `gen_drive` text in order, then the runtime's `drive_builtins` order — pulls, switches, clocks, resets R then /R, port-stimulus input columns in column order — then memory data bits, memories in table order, bits 0..w−1). It stable-sorts the drivers by net and numbers them, so each net's slots are contiguous and stay in contribution order. A driver on no net (−1) gets no slot. **Generated tables:** `gen_slot_count`; `gen_net_slot_start[gen_net_count+1]` (CSR: net n's slots are `[start[n], start[n+1])`); `gen_slot_label[]` (a `gen_labels` index per slot); `gen_slot_weak[]` (the pull tier, FR-083); `gen_slot_net[]`. `gen_max_contribs` is removed. The built-in tables carry slots in place of labels: `rt_pull.slot`, `rt_switch.slot`, `rt_clock.slot` (`rt_clock.label` is kept for the vector runner's hidden-clock check, since an unwired clock has no slot), `rt_reset.r_slot`/`rn_slot`, `rt_mem.data_slot` (replacing `data_label`), and `rt_incol.slot` for port kinds (−1 otherwise). **Drive:** generated code writes `rt_drive(slot, v)`, a `static inline` store into `rt_slot_val[]` in `runtime.h`. A driver on no net emits nothing. Every runtime driver writes its slot every step, `RT_Z` when not driving: a deselected or disabled memory writes Z to its data slots, and an unarmed port stimulus writes Z. `reset_state` sets every slot to Z. **Resolution:** `resolve_net(n)` scans the net's slots in order, skipping Z, and tallies the strong and weak tiers exactly as before. It keeps the **first** 0-driving and the **first** 1-driving label, which is what the old newest-first list walk reported, so conflict messages are unchanged. A net with exactly one slot takes that slot's value directly (a lone driver cannot conflict, and a lone pull decides alone), and a net with no slots is Z. Double buffering, `rt_step`'s return value, and the FR-117d fixed-point test are unchanged. **Interface change:** `rt_contrib` is gone and the table layouts changed, so a program generated earlier needs its own `runtime.c` until it is regenerated.
  13. **M13 — event-driven evaluation** (scoped 2026-09-24: FR-110a). After M12 the `examples/cpu` profile was about 74% `gen_drive`: every step recomputed all ~700 drivers, although only about 10 of 399 nets change in a typical step. **Units:** each generated driver is one **evaluation unit** (a GALasm output with its `.E` gating and LHS negation, a decoder output, or a constant-U driver of FR-080), numbered in `gen_drive` text order. A driver on no net has no slot and no unit. While lowering a unit, the generator records every net its expression reads (each literal through `litExpr`, or the decoder's literal helper) and whether it reads instance state: `regprev_`/`reg_` (`.R` feedback and output) or `latch_` (`.L` output). **Generated interface:** `gen_drive` is replaced by `void gen_eval(int unit, const rt_val *curr)`, a `switch` with one case per unit, each ending in `rt_drive(slot, v)`, plus `gen_unit_count`. It also emits a net fanout CSR table, `gen_net_fan_start[gen_net_count+1]`/`gen_net_fan[]`: the units reading each net, deduplicated. In `gen_latch`, each register instance and each latch instance keeps a local change flag. When the flag is set, the block calls `rt_mark_units(deps_reg_<tag>, N)` (or `deps_latch_<tag>`), the units reading that state, and sets the returned `ch` as before. **Runtime:** there are two dirty sets, each a flag array plus a list: units, and nets. `rt_drive` becomes an inline compare-and-store: a slot whose value actually changes marks its net dirty. `rt_step` then runs: (1) the latch phase, unchanged, which marks state-dependent units; (2) `gen_eval` for each dirty unit, then clear the unit set; (3) `drive_builtins` and `mem_drive_all`, still every step, because they are few and only real changes mark nets; (4) resolve each dirty net straight into `curr`, which is safe because resolution reads only slots and every reader of this step's `curr` has already run. A net whose value changes sets the return flag and marks its fanout units dirty for the next step. Then clear the net set, `sim_time++`, `vcd_sample()`, and set `step_fixed` as before. `next_buf` and the swap are gone. **Exactness:** a unit's slot is a pure function of `curr` and its instance state, so it can differ from its last evaluation only if one of those changed. A net's resolution (value and conflict flag) is a pure function of its slots. **Reset:** `reset_state` sets nets and slots to Z, clears both dirty sets and `conflicted`, then marks every unit dirty, so the first step after power-up, and after each combinational vector row's reset, evaluates everything. The FR-117d skip, `next_builtin_change`, SIGINT handling, the vector runner and the VCD writer are unchanged: after a fixed-point step both dirty sets are empty. **Interface change:** a program generated before M13 needs its own `runtime.c` until it is regenerated.
...
stdout, then `fflush(stdout)` if the byte was `'\n'`, so line-oriented output reaches the
console as it is produced (amended 2026-09-24). `main()` sets `setvbuf(stdout, NULL, _IOFBF, …)` (heavily buffered) and does
...
| `clock` | string? | optional name of the input pin that clocks `.R` registered behavior outputs (FR-062d); parser-validated to exist with `dir: in`; required (checked at Run time) iff the behavior uses `.R`. On a GAL part it is the author's explicit pin-1 clock designation (FR-066i), independent of the behavior, and a bad value is a load error rather than a rejection (FR-066j) |
...
| `loadErrors` | `string[]?` | GAL parts only (FR-066j): the problems the parser found and, because the file declares `gal:`, **collected instead of rejecting** it (§6.3). One input to `galDefinitionErrors` (§6.14); absent when empty, and always absent on a non-GAL type. Rides along in the `typeData` copy saved with a design (FR-057) like every other field, so a copy's errors describe that copy until Refresh Types (FR-088) replaces it |
| `extra` | `object?` | GAL parts only (FR-066j): top-level YAML keys the parser maps onto no field, decoded, so the Edit GAL part dialog writes them back rather than dropping them. Read by nothing else |
...
| `olmc` | `"reg"`\|`"comb"`? | GAL parts only (FR-066i): the declared type of an output-configured OLMC pin, independent of the behavior block; absent ⇒ inferred from whether the behavior writes the pin with `.R`. Ignored on non-GAL types |
| `unplaced` | bool? | GAL parts only (FR-066j): the pin's `side`/`pos` was unusable at load. Carried for the dialog; skipped by drawing, hit-testing and wiring |
...
| `pins[].olmc` | no | `Pin.olmc` | GAL parts only (FR-066i): `reg` \| `comb`, the declared type of an output-configured OLMC pin. Written by the GAL dialog on every output OLMC, with or without an equation; absent ⇒ inferred from whether `behavior` writes the pin with `.R`. Must be on a pin whose `dir` is not `in` (a load error otherwise, FR-066j). Ignored on non-GAL types |
...
**GAL parts load leniently (FR-066j).** A file declaring `gal:` is refused only
when it is not valid YAML. Any other rule in the table above that it breaks
becomes a definition error carried on the loaded type (§6.3; `loadErrors`,
§7.1), the part loads, and its placed instances draw red (FR-066l). A file
without `gal:` is held to the table exactly as before.
...
| GAL output type and clock: where the intent lives (FR-066i) | (a) keep inferring registered-ness from `.R`, and emitting `clock:` iff some output is registered; (b) a new `dir` value (`reg`); (c) a per-pin `olmc: reg\|comb` key plus an authored `clock:` | **(c)** | (a) is the reported bug: a part blocked out before its logic loses its output types on save, and its `clock:` then contradicts the inference, so it cannot reopen. (b) would push a GAL-only notion into the `dir` vocabulary that every part, the netlist, the DRC and both engines read. (c) is additive (FR-066) and GAL-scoped and leaves `dir` meaning what it means everywhere; a pin without the key keeps the old inference, so no existing part must migrate |
| Inconsistent GAL definitions (FR-066j) | (a) reject at load, refuse to open, block Save until consistent (the prior rules); (b) accept and silently normalize on save; (c) accept, keep what cannot be represented, and report conflicts as errors computed from the definition | **(c)** | The author's process runs through inconsistent states for long stretches (pinout before logic), so (a) blocks legitimate work and sends the user to a text editor, and (b) destroys intent. Errors computed from the definition — never stored — cannot go stale, and one `galDefinitionErrors` shared by the dialog, the canvas and the run/generate/test-vector refusals keeps every surface in agreement. GAL-only: nothing authors 74-series files in-app |
| What counts as a GAL definition error (FR-066j) | Every gap (a declared output with no equation, no clock yet, content the dialog cannot edit) as well as every conflict | **Conflicts only; gaps simulate at U (FR-080) and kept content is a load note** | Counting gaps would paint every part in progress red and leave the design unrunnable for the length of the design process — the opposite of the workflow this serves. Counting kept content would draw the hand-written shared 22V574 (`.E`, `AR`, `SP`) red for a limitation of the dialog rather than a fault in the part |
...
| Magic UART: fast-C output channel (FR-122d) | A separate file/fd; stderr; **real stdout, fully buffered** | **stdout (`setvbuf _IOFBF`), flushed on each UART newline** | The prompt specifies "standard output"; full stdio buffering is the "heavily buffered" requirement. The newline flush (added 2026-09-24) makes line-oriented output visible in an unbounded free run, at the cost of one write per line. Interleaving with the trailing free-run dump/vector transcript is acceptable (dump trails all UART output) and handled by the parity harness's ordering |
...
web/
  ...
  js/engine/galeq.js        GAL equation term-table model ↔ GALasm text (§6.11, FR-066g)  [CREATE]
  js/engine/galerrors.js    GAL definition errors — one check for dialog, canvas, Run/Generate C/test vectors (§6.14, FR-066j–m)  [CREATE]
...
| FR-006b | §6.11 | `app.js`, `contextmenu.js`, `interaction.js` |
...
| FR-114h | §6.6, §6.15, §6.10, §6.9 | `model/design.js` (`derivedRamFile`/`ramFileOf`/`takenRamFiles`, `addInstance`, `refreshInstance`), `model/clipboard.js`, `commands.js`, `engine/interaction.js` |
| FR-114i | §6.11, §6.10 | `chrome/properties.js`, `commands.js` (`setMemFileCmd`), `chrome/dialogs.js` (`openFileDialog`), `css/style.css` |
...
| FR-106–FR-110, FR-116, FR-116a, FR-117, FR-117a, FR-117b, FR-117c, FR-117d, FR-118 | §6.17 | `engine/cgen.js`, `web/cgen/runtime.h`, `web/cgen/runtime.c`, `chrome/toolbar.js`, `app.js` |
...
| FR-111, FR-112, FR-112a, FR-113 | §6.15, §6.9, §6.10, §6.11 | `clipboard.js`, `interaction.js`, `commands.js`, `canvas.js`, `toolbar.js` |
...
| FR-066i, FR-066j, FR-066k, FR-066l, FR-066m | §6.3, §6.4, §6.13, §6.14, §6.17, §7.1, §7.6, §8 | `yamlparse.go`, `types.go`, `api.go`, `engine/galerrors.js`, `engine/galeq.js`, `chrome/dialogs.js`, `engine/canvas.js`, `engine/interaction.js`, `engine/sim.js`, `engine/cgen.js`, `app.js`, `css/style.css` |
...
- **JS `resolveSubDesigns` stale attachments (FR-099b):** after a child's ports
  are renamed, a wire's `pin` vertex on the instance becomes a `free` one with
  a report, **and** a bus `groupConnections` entry whose `bitMap` names any
  vanished pin is dropped whole with a report — the bus, its endpoint vertex,
  and group connections to other instances all survive, and a group whose pins
  still exist is untouched (no report). The dropped case is the 2026-08-10
  regression: every surviving group connection must resolve through
  `busGroupBrace`, which the canvas calls per frame.
...
- **JS Edit GAL part (FR-066f):** `galPartFromType` round-trips a created part's
  ...
  create → parse → load back → re-serialize → **byte-identical YAML**; since FR-066j it
  **never refuses**: an off-skeleton pin set, an unmodelled key, an unparseable
  equation and a `clock:` with no registered output each load with a load note
  and survive Save in their kept form. The palette menu offers the
  item for a `projectLocal` GAL22V10 only (not shared, not memory, not 74-series, not
...
- **JS/Go GAL definition errors (FR-066i–FR-066m; `galerrors.test.js`,
  `dialogs.test.js`, `yamlparse_test.go`):** the reported bug is the first case — a
  part with ten `olmc: reg` outputs, `clock:` set and **no** `behavior` goes create →
  parse → load back → re-serialize **byte-identical**, reopens with every OLMC
  still "reg out" and the box checked, and has **no** definition errors. Each
  conflict yields exactly one error (`olmc: reg` without `.R`, `olmc: comb` with
  `.R`, `olmc: reg` or `.R` with no `clock:`, an equation on an input OLMC, the
  clock signal as a literal); a file with no `olmc` keys yields none of the
  mismatch errors; the shared-library `22V574.yaml` yields `[]`. In Go, each former
  GAL rejection (bad clock pin, unknown or `NC` group member, missing
  `partnumber`, bad `gal:`, bad `olmc`, physical incompleteness) now loads with one
  `loadErrors` entry, undecodable YAML still errors, and the same faults in a
  non-GAL file still reject. `designDefinitionErrors` names a top-level GAL by
  refdes and a sub-design instance by its own refdes when a GAL two levels down is
  bad. The FR-080 per-pin case — a GAL whose behavior writes some outputs and not
  others — drives U on the unwritten ones identically in `sim.js` and generated C
  (`sim.test.js`, `cgen.test.js`).
...
- **JS GAL equation term table (FR-066g, `galeq.test.js`):** the round trip is
  ...
  it does **not** claim each come back as `kept` equations, emitted verbatim after
  the table's: `.E`/`AR`/`:+:`, an LHS
  negation disagreeing with its pin label, self-feedback, and a both-polarities
  term that is *not* the output's only term (the one that *is* loads as
  `const0`). Emission rules get their own cases: an all-X row is skipped rather
  than written `VCC`, an output whose rows are all empty emits nothing, `const0`
  /`const1` write `GND`/`VCC`, outputs come out in pin order 14→23, a `/ENF` pin
  heads `/ENF =` while its literals read `ENF`, `.R` follows the output's own `reg` flag rather than the OLMC's declared type,
  and a per-output note survives text → table → text. Keying is tested by
  relabeling a pin between parse and emit: every term that named it follows the
  new label, since the model holds DIP numbers.
...
- **JS debug-input ports (FR-094g):** `debugPorts` eligibility — an unwired port and
  one whose net only feeds inputs qualify (`in`); a port on a net driven by an output
  pin does not (`out`); one on a net touching a bidir/tristate pin does (`bidir`),
  including when a `dirOverride` narrows it; a port carrying a `target` never does; a
  `portN` maps to its bit count. Drive semantics in `sim.js`: an undriven port
  contributes nothing and the net resolves exactly as it does today (the regression that
  guards "no click, no change"); driven to 1/0 it is a **strong** driver that beats a
  pull-up and conflicts with an opposing switch; a 1-wide port's contribution lands on
  its **label-joined** net (FR-094a/FR-094e), and each `portN` bit only on its own. The
  handler cycles undriven → 1 → 0 → undriven per bit and leaves its neighbors alone; a
  built simulation with no `liveInputs` accessor (the vector runner, the parity harness)
  sees every port undriven.
