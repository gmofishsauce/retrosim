...
*Model.* `{ outputs: Map(olmcNumber → { mode:"eq"|"const0"|"const1", note, rows:[ Map(pinNumber → "1"|"0") ] }) }`. Everything is keyed by **skeleton DIP number**, never by label — the same rule that keeps a pin group intact across a rename (FR-066d) — so relabeling a pin mid-dialog preserves every term that names it; labels are resolved only when the text is emitted. A row is a sparse map: a signal absent from it is the `X` cell, so an empty row is an empty map, and a term is never larger than the literals actually in it.

*Emit.* `tableToBehavior(model, { labelOf, regOf })` walks the OLMC pins in pin order 14→23 and writes each output that has something to say: `const0`/`const1` emit `LHS = GND` / `LHS = VCC`; `eq` drops empty rows and, if any survive, emits one AND term per line joined by leading `+`, with `*` between literals and `!` for a `"0"` cell. The LHS is the pin label verbatim (so a `/ENF` pin heads `/ENF = …`, FR-066g) plus `.R` when the OLMC is a registered output; RHS literals use the label with any leading slash stripped, §7.6's physical-level convention. The output's note, if any, is appended to its first line as `; …`. An output with no surviving rows emits nothing at all.

*Parse.* `behaviorToTable(text, { pins })` is the inverse and the second refusal surface of the Edit flow (FR-066f). It cannot reuse `galasm.js`'s `tokenize`, which discards `;` comments and line structure — both of which the table keeps (FR-066g's per-output notes) — so `galeq.js` carries a small line-aware tokenizer of its own and a deliberately **narrow** recognizer: only `[/|!]NAME[.R] = <sum of products>` and `NAME = VCC|GND` are accepted, which is what makes every unsupported form (`.E`, `.T`, `.L`/`.G`, `.CLK`/`.ARST`/`.APRST`, `:+:`, `AR`, `SP`) a refusal rather than a silent drop. It further refuses an LHS whose negation disagrees with its pin's label (the polarity lives in the label, FR-066g, so loading `!OUTCMB = …` on a pin labeled `OUTCMB` and saving would invert the output), an output whose term reads its own signal back, and a term naming one signal in **both** polarities — except where that contradiction is the output's only term, which is exactly the idiom for a hard 0 (`S2 = F0 * !F0`) and loads as `const0`. A literal repeated in the same polarity within a term collapses onto its one cell. Comments are attached to the output whose equation encloses or follows them, the trailing ones to the last output. The equations the recognizer accepts are then re-validated by the ordinary live gate, so `galeq.js` never has to duplicate `compileBehavior`'s judgment — it only has to decline what it cannot represent.

*DOM.* `dialogs.js` builds the grid from the current labels and directions: a sticky header row of column labels (pin order, `NC` pins excluded), then per output a contiguous run of rows carrying the output's label, its equation/always-0/always-1 mode select and note field on the first row, one cycling `X/1/0` button per column, the positional **OR/done** drop-down, and the trailing **✕**. The self-column cell and — while any OLMC is registered — the pin-1 column are rendered inert. The grid is rebuilt (never re-keyed) whenever a label or direction changes, which is why the model is keyed by DIP number: rows for an OLMC switched to `in` are hidden and kept, not discarded, and come back if it is switched to `out` again before Save. Below the grid sits a **read-only** `<pre>` preview of `tableToBehavior`'s output, refreshed on every edit, and below that the unchanged live validation line — the candidate `typeData` now takes its `behavior` from `tableToBehavior` instead of from a textarea, so `compileBehavior`+`validateStrict` (§6.13) still gate Create/Save with no second validator anywhere.
...
- **JS GAL equation term table (FR-066g, `galeq.test.js`):** the round trip is
  ...
  it does **not** claim each refuse with their own reason: `.E`/`AR`/`:+:`, an LHS
  ...
  heads `/ENF =` while its literals read `ENF`, `.R` follows the OLMC direction,
