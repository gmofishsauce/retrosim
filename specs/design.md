...
**Pure runner & file model (`engine/vectors.js`, DOM-free, unit-tested in `vectors.test.js`).** A new module, deliberately free of any DOM so it tests like `validateMemSpec`/`memDeviceSpec`:
  ...
  - `reconcileVectors(fileDoc, columns) → { rows, warnings }`. The **FR-115a reconciliation**, deliberately separate from the validity gate above: it aligns a loaded file to the design's current columns by `(refdes,pin)`, returning rows in **`columns` order** — each cell pulled from the file where that column still matches, and defaulted otherwise (`"0"` for inputs, `"C"` for a `kind:"clock"` input so it matches `emptyRow`, `"X"` for outputs and io). A column on only one side is a **non-fatal warning**, one per direction of mismatch (in the file but not the design, and vice versa), so a file whose design has since gained or lost a switch still loads, carrying over the rows that align. Splitting it from `validateVectors` is what lets the same alignment serve the panel's Load, the parity harness, and `tv2txt` (§6.17 M2) without dragging the panel's cell-symbol rules along.
...
| FR-115, FR-115a–m | §6.10, §6.13, §6.16, §7.7 | `engine/vectors.js`, `engine/sim.js`, `chrome/dialogs.js`, `chrome/toolbar.js`, `chrome/properties.js`, `store.js`, `app.js`, `index.html`, `style.css` |
