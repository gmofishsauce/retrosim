...
- **Satisfies:** FR-115, FR-115a, FR-115b, FR-115c, FR-115d, FR-115f (port binding); extends FR-004a (new **Simulate** menu).
...
- **Dependencies:** `engine/sim.js` (`buildSimulation` with `stimulus`, settle bound, `loadRomContents`), `engine/galasm.js` (`V0`/`V1`/`VU`/`VZ`), `model/design.js` (instance shape), `model/subdesign.js` (`effectivePortDir` for port-column direction, FR-115f), `store.js` (`state.design`, `state.savePath`, `state.simulating`), `chrome/dialogs.js` (`openFileDialog`), `chrome/fileops.js` (`dirOf`), `api.js` (load/save), `chrome/toolbar.js`, `app.js`.
