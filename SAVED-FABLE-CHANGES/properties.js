...
export function initProperties({ container, store }) {
  function render() {
    ...
    if (td.renderType === "port" || td.renderType === "portN") {
      ...
      // (a path relative to this design's save directory, FR-098) and the port
      // label within that file (FR-101a). A non-empty file sets/updates
      // inst.target; clearing the file clears it (the label alone never
      // creates a target). Both edits ride setPortPropsCmd, so they are
      // undoable (FR-024) and honor the locks like the label field.
      if (td.renderType === "port") {
        ...
        fileInput.placeholder = "peer sheet .json (relative)";
        fileInput.title =
          "Off-sheet target design file, relative to this design's folder; blank = no target";
        fileInput.disabled = locked;
        fileInput.addEventListener("change", () => {
          commitTarget(fileInput.value.trim(), inst.target?.label ?? "");
        });
