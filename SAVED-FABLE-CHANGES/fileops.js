...
export function makeFileOps({ store, dataDir, defaultName, onNavChange = () => {}, onLoaded = () => {} }) {
  ...
  // the target file is stored relative to this design's directory (FR-098), so
  // a never-saved design is prompted to save first — the relative path needs a
  // directory to resolve against, and back (FR-100a) needs a file to return to
  // (the same interim rule as descend). On success the referring sheet joins
  // the back-stack exactly like a descent.
