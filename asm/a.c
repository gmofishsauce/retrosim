...
/* like get_label_address(), but an undefined label is a hard error */
short
label_address(char *s)
{
	short addr = get_label_address(s);
...
main(int argc, char *argv[])
{
	...
	while(readAndParse(inFilePtr, lineString, &label, &opcode, &arg0, &arg1, &arg2) != NULL) {
		...
		/* a label on a line by itself occupies no space */
		if (opcode == NULL) {
			continue;
		}
	...
	while(readAndParse(inFilePtr, lineString, &label, &opcode, &arg0, &arg1, &arg2) != NULL) {
		...
		if (opcode == NULL) {
			/* a label on a line by itself emits nothing */
			continue;
			...
			if (isNumber(arg2)) {
				...
				immediateValue = label_address(arg2) - address - 1;
			}
			...
			fprintf(stderr, "error: unrecognized opcode [%s] at address %hd\n", opcode, address);
			exit(1);
		}
